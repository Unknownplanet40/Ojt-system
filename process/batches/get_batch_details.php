...
if (empty($batchUuid)) {
    response(['status' => 'error', 'message' => 'Batch UUID is required.']);
}

$batchDetails = getBatch($conn, $batchUuid);

if (!$batchDetails) {
    response(['status' => 'error', 'message' => 'Batch not found.']);
}

response(['status' => 'success', 'data' => $batchDetails]);
