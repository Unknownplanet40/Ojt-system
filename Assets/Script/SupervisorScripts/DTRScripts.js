...
const state = { entries: [], selected: new Set(), active: null, geofence: null };
...
function renderSummary() {
  $('#supervisorEntriesCount').text(state.entries.length);
  $('#supervisorStudentsCount').text(state.assignedCount || 0);
  $('#supervisorPendingCount').text(state.entries.filter((e) => e.status === 'pending').length);
  $('#supervisorBackdatedCount').text(state.entries.filter((e) => e.is_backdated).length);
}
...
function filteredEntries() {
  ...
  const backdated = $('#supervisorBackdatedFilter').val();
...
function renderEntries() {
        ...
        <div class="card-body">
          <div class="d-flex flex-column flex-lg-row justify-content-between align-items-start gap-3 dtr-entry-header">
            <div class="d-flex gap-3 flex-grow-1 align-items-start">
              <div class="pt-1">
                ${isPending ? `<input class="form-check-input supervisor-entry-check" type="checkbox" data-uuid="${entry.uuid}" ${checked}>` : `<span class="d-inline-flex align-items-center justify-content-center rounded-circle bg-secondary-subtle text-secondary-emphasis" style="width: 1.2rem; height: 1.2rem;"><i class="bi bi-dot"></i></span>`}
              </div>
  ...
  $('#selectAllsupervisorEntries').prop('checked', filtered.filter((entry) => entry.status === 'pending').length > 0 && filtered.filter((entry) => entry.status === 'pending').every((entry) => state.selected.has(entry.uuid)));
}
...
function loadDtr() {
  $.ajax({
    ...
    data: {
      ...
      month: $('#supervisorMonthFilter').val() || '',
      is_backdated: $('#supervisorBackdatedFilter').val(),
    },
    success: (response) => {
      ...
      state.entries = [...(response.entries || []), ...(response.history || [])];
...
function openDecision(entry) {
  state.active = entry;
  $('#supervisorDecisionStudentName').text(entry.full_name);
  $('#supervisorDecisionStudentLabel').text(entry.full_name);
  $('#supervisorDecisionStudentNumber').text(entry.student_number || '—');
  $('#supervisorDecisionProgram').text(entry.program_code || '—');
  $('#supervisorDecisionEntryDate').text(entry.entry_date_label);
  $('#supervisorDecisionTimeRange').text(`${entry.time_in_label} - ${entry.time_out_label}`);
  $('#supervisorDecisionHours').text(entry.hours_label);
  $('#supervisorDecisionStatus').text(entry.status_label || 'Pending');
  $('#supervisorDecisionBackdated').text(entry.is_backdated ? 'Yes' : 'No');
  $('#supervisorDecisionActivities').text(entry.activities || '—');
  $('#supervisorDecisionSubmittedAt').text(entry.submitted_at || '—');
  $('#supervisorDecisionReason').val(entry.backdate_reason || '');

  const verificationContainer = $('#supervisorDecisionVerificationContainer');
  const selfieContainer = $('#supervisorDecisionSelfieContainer');
  const selfieImg = $('#supervisorDecisionSelfie');
  const gpsContainer = $('#supervisorDecisionGpsContainer');
  const gpsCoords = $('#supervisorDecisionGpsCoords');

  const mapContainer = $('#supervisorDecisionMapContainer');
...
function renderGeofenceMapSupervisor(entry) {
  setTimeout(() => {
    const mapElement = document.getElementById('supervisorDecisionMapContainer');
    if (!mapElement) {
    ...
    try {
      mapInstance = L.map('supervisorDecisionMapContainer').setView([geofence.latitude, geofence.longitude], 17);
    } catch (e) {
    ...
    if (entry.clock_in_latitude && entry.clock_in_longitude) {
      ...
      let popupHTML = `
        <div>
          ...
          <small class="text-muted">${entry.full_name}</small><br>
      `;
...
function rejectEntry(entry) {
  const reason = $('#supervisorDecisionReason').val().trim();
  if (!reason) {
...
$(document).ready(() => {
  ...
  $('#clearSupervisorFiltersBtn').on('click', () => {
    ...
    $('#supervisorBackdatedFilter').val('');
    renderEntries();
  });

  $('#supervisorSearchInput, #supervisorStatusFilter, #supervisorMonthFilter, #supervisorBackdatedFilter').on('input change', renderEntries);

  $('#selectAllSupervisorEntries').on('change', function () {
    const checked = $(this).is(':checked');
    $('#supervisorDtrList .supervisor-entry-check').toArray().forEach((el) => {
  ...
  $('#supervisorDtrList').on('click', 'button[data-action="review"], button[data-action="view"]', function () {
  ...
  $('#supervisorDtrList').on('click', 'button[data-action="reject"]', function () {
    const entry = state.entries.find((item) => item.uuid === $(this).data('uuid'));
    if (entry) openDecision(entry);
    $('#supervisorDecisionReason').focus();
  });

  $('#supervisorDecisionApproveBtn').on('click', () => {
    if (state.active) approveEntry(state.active);
  });

  $('#supervisorDecisionRejectBtn').on('click', () => {
