    ...
    steps: [
      {
        ...
        popover: {
          title: "Supervisor Profile Setup",
      ...
      {
        element: ".position-group",
        popover: {
          title: "Required Information",
          description: "Enter your position so students and coordinators know your role.",
          side: "top",
          align: "start",
        },
      },
...
const ACTION_STORAGE_KEY = "supervisor_profile_action";
...
function ProfileProgressBar(fill = 0) {
  ...
  if ($("#position").val().trim()) filledFields++;
...
$(document).ready(function () {
  ...
  $("#firstName, #lastName, #position, #contactNumber, #department").on("input", function () {
    ProfileProgressBar();
  });
      ...
      success: function (response) {
        if (response.status === "success") {
          ...
          if (response.redirect_url && setupProfile) {
            ...
            window.location.href = "../../../Src/Pages/Supervisor/SupervisorDashboard";
          }
        } else {
